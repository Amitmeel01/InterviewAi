...
function Feedback({ params: { interviewId } }: any) {
    ...
    useEffect(() => {
        async function fetchFeedback() {
            try {
                const data: any = await feedbackHandler(interviewId);
                console.log(data);
    ...
    if (!feedbackData.length) {
        return  <div className="flex flex-col items-center justify-center h-full p-8 bg-gray-100 mt-32">
        <div className="bg-white shadow-md rounded-lg p-6 text-center">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">
            No Feedback Available
          </h2>
          <p className="text-gray-500 mb-8">
            Please give an interview first to receive feedback for this role.
          </p>
          <Link href="/dashbord">
            <Button className="bg-blue-600 text-white mt-4 px-6 py-3 rounded-lg">
              Go Home
            </Button>
          </Link>
        </div>
    ...
    return (
      ...
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-4xl">
        ...
        <h2 className="text-purple-500 text-lg my-3 font-bold">
          Your Overall Interview Rating: {overallRating?.toFixed(0)}/10
        </h2>
        <h2 className="text-md text-zinc-600 mb-6">
          ...
          <Collapsible key={index} className="mb-4">
            ...
            <CollapsibleContent className="p-4 bg-white border-t border-gray-200">
              <div className="flex flex-col gap-4">
                <h2 className="text-yellow-600 font-bold p-2 border rounded-lg">
                  Rating: {item.rating}
                </h2>
                <h2 className="text-red-600 font-bold p-2 border rounded-lg bg-red-100 text-sm">
                  Your Answer: {item.userAns}
                </h2>
                <h2 className="text-green-600 font-bold p-2 border rounded-lg bg-green-100 text-sm">
                  Correct Answer: {item.correctAnswer}
                </h2>
                <h2 className="text-yellow-600 font-bold p-2 border rounded-lg bg-yellow-100 text-sm">
